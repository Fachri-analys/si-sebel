...
class Settings(BaseSettings):
    ...
    phone_hash_key: str = Field(
        default="",
        description="Secret key used to pseudonymize phone numbers"
    )
    ...
    security_mode: str = Field(
        default="strict",
        description="Security mode (strict, permissive)"
    )
    ...
    max_log_length: int = Field(
        default=500,
        description="Maximum log entry length"
    )
    ...
    max_instances: int = Field(
        default=1,
        description="Maximum number of bot instances"
    )
    max_connections: int = Field(
        default=50,
        description="Maximum concurrent connections per instance"
    )
    max_requests: int = Field(
        default=100,
        description="Maximum requests per time window (rate limiting)"
    )
